...
export default function UserLogin({ role, setRole }) {
    ...
    return (
        <div className="flex flex-col gap-5 items-center justify-center bg-slate-950 bg-no-repeat bg-center bg-contain " style={{ minHeight: "calc(100vh - 4rem)" }}>
            <form className='bg-gradient-to-t from-slate-900 via-cyan-950 to-slate-950 shadow-[0_0_7px_white] w-[98%] max-w-[800px] py-8 rounded-xl min-h-[280px] opacity-90 flex flex-col gap-3' autoComplete='on' onSubmit={handleUserLogin}>
                <h1 className='mx-auto py-1 text-center text-4xl rounded-md font-extrabold drop-shadow-[0_0_5px_black] text-transparent bg-clip-text bg-gradient-to-r from-pink-500 to-violet-500 w-fit'>Login Page</h1>
                ...
                <div className="w-[95%] max-w-[600px] m-auto flex justify-between">
                    <Link href={`/registerUser`} className='w-[130px] xs:w-[160px] text-center rounded-md bg-red-900 py-2 hover:bg-red-700 active:bg-indigo-900 font-mono' >Resister New</Link>
                    <button type='submit' className='w-[130px] xs:w-[160px] text-center rounded-md bg-red-900 py-2 hover:bg-red-700 active:bg-indigo-900 font-mono' >Login</button>
                </div>
            </form>
        </div>
    )
