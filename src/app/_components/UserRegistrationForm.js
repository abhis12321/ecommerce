...
export default function UserRegistrationForm({ setOption }) {
  ...
  const handleUserRegistration = e => {
    e.preventDefault();
    const body = { user_name, user_gender, user_role, email, password, country_code, mobile_no };
  ...
  return (
    <form className='w-[92%] max-w-[600px] pb-4 text-gray-600 bg-white shadow[0_0_4px_black] rounded-2xl gap-1 flex flex-col items-center justify-center' autoComplete='on' onSubmit={handleUserRegistration}>
      <div className="flex flex-row sm:flex-row gap-2 items-center justify-evenly w-[98%] max-w-[600px]">
        <select name="status" value={user_gender} onChange={(e) => setGender(e.target.value)} className='w-1/2 outline-none invalid:text-pink-600 font-semibold shadow-[0_0_3px_gray_inset] bg-slate-950/10 hover:bg-slate-950/15 focus:bg-blue-600/20 py-[6px] px-3 rounded-md text-center' >
        ...
        <select name="status" value={user_role} onChange={(e) => setRole(e.target.value)} className='w-1/2 outline-none invalid:text-pink-600 font-semibold shadow-[0_0_3px_gray_inset] bg-slate-950/10 hover:bg-slate-950/15 focus:bg-blue-600/20 py-[6px] px-3 rounded-md text-center' >
